...
function DetailRow({ icon: Icon, label, value, labelClass, valueClass }) {
  return (
    <div className="flex items-center gap-3">
      <Icon className="h-4 w-4 shrink-0 opacity-70" strokeWidth={1.75} aria-hidden="true" />
...
export default function CompanyCard({
  ...
  return (
    <m.div
      className={`relative h-full cursor-pointer overflow-hidden rounded-sm border transition-opacity duration-300 ${surfaceClasses(
        company.logoTone
          ...
          <div className="pt-7">
            ...
            <div className="mt-6">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                <span className={`min-w-[3.25rem] rounded-sm border border-current px-3 py-1 text-center text-caption font-semibold tabular-nums ${text.primary}`}>
                  {standValue}
                </span>
              </div>
            </div>
            ...
            <a
              ...
              className={`mt-6 inline-flex items-center gap-2 font-semibold underline underline-offset-4 transition-[gap,color] duration-settle ease-settle hover:gap-3 ${text.primary}`}
            >
              {t.visitWebsite}
