...
function CompanyRow({ company, edition, isHighlighted, showMapLink }) {
  ...
  return (
    <div
      ...
      className={`overflow-hidden rounded-sm border p-5 transition-colors duration-settle ease-settle ${surfaceClasses(
        company.logoTone
      )} ${
        isHighlighted
      ...
      <div className="grid gap-4 sm:grid-cols-[140px_1fr_auto] sm:items-start">
        ...
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className={`rounded-sm px-3 py-1 text-xs font-semibold ${text.chip}`}>
              {labels[company.category] ?? company.category}
            </span>
            {hasStand ? (
              <span className={`text-caption font-medium tabular-nums ${text.secondary}`}>
        ...
        <div className="flex flex-col items-start gap-2 sm:items-end">
          {hasStand && showMapLink ? (
            <Link
              ...
              className={`inline-flex min-h-11 items-center gap-2 rounded-sm border px-4 text-small font-medium transition-[transform,background-color,border-color,color] duration-settle ease-settle active:scale-[0.97] active:duration-press ${mapButtonClasses(
                company.logoTone
              )}`}
            >
              {t.showOnMap}
  ...
  return (
    <PageLayout title={t.companiesPageTitle} edition={edition} wide>
      ...
      {allCompanies.length === 0 ? (
        ...
        <>
          {/* -- Filter bar -- */}
          <div className="flex flex-col gap-3 rounded-md border border-navy/[0.07] bg-navy/[0.02] p-4 dark:border-white/10 dark:bg-white/5 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-2 text-sm font-semibold text-navy dark:text-white">
              <Filter className="h-4 w-4 text-fg-muted dark:text-slate-400" aria-hidden="true" />
              {t.filter}
            </div>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
              <label className="flex items-center gap-3 text-sm text-slate-700 dark:text-slate-200">
                ...
                <select
                  className="rounded-sm border border-navy/15 bg-white px-3 py-2 text-body text-slate-900 transition-colors duration-settle hover:border-navy/30 focus:outline-none focus-visible:ring-2 focus-visible:ring-orange/70 dark:border-white/10 dark:bg-ink dark:text-slate-100"
                  value={category}
              ...
              <button
                type="button"
                className="rounded-sm border border-navy/15 bg-white px-3 py-2 text-body font-semibold text-slate-900 transition-[background-color,border-color] duration-settle ease-settle hover:border-navy/25 active:scale-[0.98] active:duration-press dark:border-white/10 dark:bg-ink dark:text-slate-100"
