...
export const NOVEMBER_2026_CONTENT = {
  ...
  /** Shown after the edition name, top right: "Novembereditie 2026". */
  year: "2026",

  /**
   * The huge outlined date at the top of the footer, which lights up orange
   * around the mouse. Purely graphic; screen readers skip it. Set to null to
   * show "T.I.S." there instead.
   */
  bigDate: "26.11.26",

  /**
   * The full description — used for search engines (the page description)
   * and the calendar entry. The hero shows the shorter `lede` below, because
   * the date and venue already sit right next to it.
   */
  subtitle: {
  ...
  /** The paragraph under the rule in the hero. */
  lede: {
    nl: "Ontmoet bedrijven, ontdek stages en starterskansen, en blijf hangen voor de netwerkborrel.",
    en: "Meet companies, explore internships and graduate roles, and stay for the networking drinks."
  },

  // ── Wanneer / Waar, next to the paragraph ──────────────────────────────────

  /** "Wanneer" and the line under it. */
  whenValue: { nl: "Donderdag 26 november", en: "Thursday 26 November" },
  whenDetail: { nl: "13:00–19:00 · borrel vanaf 16:00", en: "13:00–19:00 · drinks from 16:00" },

  /** The third action under the hero (only when showAddToCalendar is true). */
  dateTooltip: { nl: "Voeg toe aan agenda", en: "Add to calendar" },

  // ── The actions under the hero ─────────────────────────────────────────────
  ...
  /** The section's name in the left rail, next to its number. */
  ...
  /**
   * The programme, drawn as a time axis from `start` to `end` with one lane
   * per part of the day (like a Gantt chart). `marks` are the times printed
   * above the axis. Whole hours only.
   * `tone`: "navy" for the stands, "orange" for the borrel.
   * Lanes that follow each other (13–16, 16–19) sit on one line; a lane
   * that overlaps another drops to a line of its own.
   * ADD A LANE: copy a { } block and set its from/to.
   */
  updateTimeline: {
    start: "13:00",
    end: "19:00",
    marks: ["13:00", "16:00", "19:00"],
    lanes: [
      {
        from: "13:00",
        to: "16:00",
        ...
        title: { nl: "Bedrijvendag", en: "Career day" },
        body: {
          nl: "Iedereen staat bij zijn stand. Loop langs, stel je vragen en spreek recruiters en engineers van de deelnemende bedrijven.",
          en: "Everyone is at their stand. Walk past, ask your questions, and talk to recruiters and engineers from the participating companies."
        }
      },
      {
        from: "16:00",
        to: "19:00",
        tone: "orange",
        title: { nl: "Netwerkborrel", en: "Networking drinks" },
        body: {
          nl: "Het informele deel, met een drankje erbij. Bedrijven zijn niet meer per se bij hun stand: ze lopen ook rond, dus spreek ze gewoon aan.",
          en: "The informal part, with a drink in hand. Companies are no longer tied to their stand: they walk around too, so just go and say hello."
        }
      }
    ]
  },

  /** The full-width "Voor bedrijven" link row under the timeline. */
  updateCompanies: {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  //  "OVER HET EVENT" SECTION  —  statement on the left, facts on the right
  // ═══════════════════════════════════════════════════════════════════════════

  /** The section's name in the left rail, next to its number. */
  aboutKicker: { nl: "Over het event", en: "About the event" },
  ...
  /** The note with the orange rule under the intro. Set to null to remove it. */
  aboutNotice: {
  ...
  /**
   * The list of facts on the right, separated by hairlines.
   *   label  the small grey word on top ("Wanneer")
   *   title  optional first line, in the larger size
   *   body   the rest
   * ADD A ROW: copy a { } block.
   */
  aboutCards: [
    {
      label: { nl: "Wanneer", en: "When" },
    ...
    {
      label: { nl: "Studenten", en: "Students" },
      body: {
    ...
    {
      label: { nl: "Bedrijven", en: "Companies" },
      body: {
