import { m } from "framer-motion";
import {
  BriefcaseBusiness,
  Clock3,
  Languages,
  MapPin,
  Moon,
  Sun
} from "lucide-react";
import { Link } from "react-router-dom";
import { useLanguage } from "../../shared/LanguageContext.jsx";
import { useTheme } from "../../shared/ThemeContext.jsx";
import {
  COMPANY_REGISTRATION_ROUTE
} from "../../shared/companyRegistration.js";

const reveal = {
  initial: { opacity: 0, y: 30 },
  whileInView: { opacity: 1, y: 0 },
  viewport: { once: true },
  transition: { duration: 0.6 }
};

function Wave({ fill = "#762222" }) {
  return (
    <svg
      className="pointer-events-none absolute bottom-0 left-0 w-full"
      viewBox="0 0 1440 120"
      preserveAspectRatio="none"
      aria-hidden="true"
    >
      <path
        d="M0,64 C240,120 480,120 720,80 C960,40 1200,40 1440,64 L1440,120 L0,120 Z"
        fill={fill}
      />
    </svg>
  );
}

function FloatingOrbs() {
  const orbs = [
    { size: 260, x: "10%", y: "20%", opacity: 0.09, delay: 0 },
    { size: 320, x: "65%", y: "15%", opacity: 0.08, delay: 0.2 },
    { size: 420, x: "35%", y: "55%", opacity: 0.07, delay: 0.4 },
    { size: 280, x: "85%", y: "60%", opacity: 0.08, delay: 0.1 },
    { size: 220, x: "5%", y: "70%", opacity: 0.07, delay: 0.3 }
  ];

  return (
    <div className="absolute inset-0 overflow-hidden">
      {orbs.map((orb, idx) => (
        <m.div
          key={idx}
          className="absolute rounded-full bg-white"
          style={{
            width: orb.size,
            height: orb.size,
            left: orb.x,
            top: orb.y,
            opacity: orb.opacity
          }}
          animate={{ y: [0, -18, 0], x: [0, 10, 0] }}
          transition={{
            duration: 10,
            repeat: Infinity,
            ease: "easeInOut",
            delay: orb.delay
          }}
        />
      ))}
    </div>
  );
}

export default function NovemberHeroSection() {
  const { lang, setLang } = useLanguage();
  const { theme, toggleTheme } = useTheme();

  const otherLang = lang === "nl" ? "en" : "nl";
  const langLabel = otherLang === "en" ? "EN" : "NL";
  const themeLabel =
    theme === "dark"
      ? lang === "nl"
        ? "Licht"
        : "Light"
      : lang === "nl"
        ? "Donker"
        : "Dark";
  const logoAlt =
    lang === "nl"
      ? "De Haagse Hogeschool"
      : "The Hague University of Applied Sciences";
  const mapsHref =
    "https://www.google.com/maps/search/?api=1&query=Rotterdamseweg%20137%2C%202628%20AL%20Delft";

  const heroSubtitle =
    lang === "nl"
      ? "De novembereditie van de Haagse Hogeschool Bedrijvendag op Delft T.I.S. Campus is in voorbereiding. De exacte datum en deelnemende bedrijven worden later bekendgemaakt."
      : "The November edition of The Hague University of Applied Sciences Career Day in Delft T.I.S. Campus is in preparation. The exact date and participating companies will be announced later.";
  const dateValue = lang === "nl" ? "Datum volgt" : "Date to be announced";
  const timeValue = lang === "nl" ? "Tijd volgt" : "Time to be announced";
  const locationValue = lang === "nl" ? "HHS Delft" : "THUAS Delft";
  const locationAddress = "Rotterdamseweg 137, 2628 AL Delft";
  const infoHint = lang === "nl" ? "Bekijk wat al bekend is" : "See what is already confirmed";
  const registrationLabel =
    lang === "nl" ? "Bedrijfsregistratie" : "Company registration";
  const primaryCta = lang === "nl" ? "Bekijk update" : "View update";
  const archiveNote =
    lang === "nl"
      ? "De editie van maart 2026 blijft beschikbaar via de editieknop linksboven."
      : "The March 2026 edition remains available through the edition button in the top-left corner.";

  const goToSection = (id, offset = 0) => {
    if (typeof window === "undefined") return;

    const element = document.getElementById(id);
    if (!element) return;

    window.history?.replaceState?.(null, "", `#${id}`);
    const rectTop = element.getBoundingClientRect().top;
    const target = Math.max(0, rectTop + window.scrollY + offset);
    window.scrollTo({ top: target, behavior: "smooth" });
  };

  return (
    <section className="relative overflow-hidden bg-gradient-to-r from-[#1e3a5f] via-[#2a4a6f] to-[#1e3a5f]">
      <FloatingOrbs />

      <div className="relative mx-auto max-w-6xl px-6 pb-24 pt-10 md:pb-28 md:pt-12">
        <div className="flex items-center justify-end gap-4">
          <img
            src="/HHS/HHS.png"
            alt={logoAlt}
            className="mr-auto h-9 w-auto max-w-[220px] opacity-95 sm:h-10 sm:max-w-[320px] md:h-12"
            loading="eager"
          />

          <div className="flex flex-col items-end gap-2">
            <button
              type="button"
              onClick={() => setLang(otherLang)}
              className="inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-2 text-sm font-semibold text-white/90 backdrop-blur-sm transition-all duration-500 hover:bg-white/15 hover:shadow-lg hover:shadow-[#f07c00]/25"
              aria-label="Switch language"
            >
              <Languages className="h-4 w-4 text-orange" />
              <span>{langLabel}</span>
            </button>

            <button
              type="button"
              onClick={toggleTheme}
              className="inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-2 text-sm font-semibold text-white/90 backdrop-blur-sm transition-all duration-500 hover:bg-white/15 hover:shadow-lg hover:shadow-[#f07c00]/25"
              aria-label="Toggle dark mode"
            >
              {theme === "dark" ? (
                <Sun className="h-4 w-4 text-orange" />
              ) : (
                <Moon className="h-4 w-4 text-orange" />
              )}
              <span>{themeLabel}</span>
            </button>
          </div>
        </div>

        <m.div {...reveal} className="mx-auto mt-14 max-w-3xl text-center">
          <p className="text-sm font-semibold uppercase tracking-[0.35em] text-white/65">
            {lang === "nl" ? "November editie" : "November edition"}
          </p>
          <h1 className="mt-4 text-5xl font-extrabold tracking-tight text-white md:text-6xl">
            <span className="text-orange">T.I.S.</span> {lang === "nl" ? "Bedrijvendag" : "Career Day"}{" "}
            <span className="text-orange">2026</span>
          </h1>
          <p className="mx-auto mt-6 max-w-2xl text-lg text-white/80 md:text-xl">
            {heroSubtitle}
          </p>

          <div className="mt-10 flex flex-col items-center justify-center gap-3 md:flex-row">
            <div className="group relative inline-flex items-center gap-3 rounded-full bg-white/10 px-5 py-3 text-white/90 backdrop-blur-sm">
              <Clock3 className="h-5 w-5 text-orange" />
              <span className="font-semibold">{dateValue}</span>
            </div>

            <button
              type="button"
              onClick={() => goToSection("november-update", -20)}
              className="group relative inline-flex items-center gap-3 rounded-full bg-white/10 px-5 py-3 text-white/90 backdrop-blur-sm transition-all duration-300 hover:bg-white/15 hover:shadow-lg hover:shadow-[#f07c00]/20 focus:outline-none focus:ring-2 focus:ring-white/50"
            >
              <Clock3 className="h-5 w-5 text-orange" />
              <span className="font-semibold">{timeValue}</span>
              <span className="pointer-events-none absolute left-1/2 top-full z-10 mt-3 w-max -translate-x-1/2 rounded-xl bg-black/60 px-4 py-2 text-sm text-white/90 opacity-0 backdrop-blur-sm transition-all duration-300 group-hover:opacity-100">
                {infoHint}
              </span>
            </button>

            <a
              href={mapsHref}
              target="_blank"
              rel="noreferrer"
              className="group relative inline-flex items-center gap-3 rounded-full bg-white/10 px-5 py-3 text-white/90 backdrop-blur-sm transition-all duration-500 hover:bg-white/15 hover:shadow-lg hover:shadow-[#f07c00]/20"
            >
              <MapPin className="h-5 w-5 text-orange" />
              <span className="font-semibold">{locationValue}</span>
              <span className="pointer-events-none absolute left-1/2 top-full z-10 mt-3 w-max -translate-x-1/2 rounded-xl bg-black/60 px-4 py-2 text-sm text-white/90 opacity-0 backdrop-blur-sm transition-all duration-300 group-hover:opacity-100">
                {locationAddress}
              </span>
            </a>
          </div>

          <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
            <button
              type="button"
              onClick={() => goToSection("november-update")}
              className="inline-flex items-center justify-center rounded-full bg-orange px-10 py-4 font-semibold text-white shadow-lg shadow-[#f07c00]/30 transition-all duration-500 hover:scale-105 hover:shadow-[#f07c00]/45"
            >
              {primaryCta}
            </button>

            <Link
              to={COMPANY_REGISTRATION_ROUTE}
              className="inline-flex items-center justify-center gap-2 rounded-full bg-orange px-10 py-4 font-semibold text-white shadow-lg shadow-[#f07c00]/30 transition-all duration-500 hover:scale-105 hover:shadow-[#f07c00]/45"
            >
              <BriefcaseBusiness className="h-5 w-5" />
              {registrationLabel}
            </Link>
          </div>

          <div className="mx-auto mt-8 max-w-2xl rounded-3xl border border-white/15 bg-white/10 px-6 py-5 text-left backdrop-blur-sm">
            <div className="text-sm font-semibold uppercase tracking-[0.24em] text-orange/90">
              {lang === "nl" ? "Archief beschikbaar" : "Archive Available"}
            </div>
            <p className="mt-3 text-sm leading-relaxed text-white/85">{archiveNote}</p>
          </div>
        </m.div>
      </div>
{/*  */}
      <Wave fill={theme === "dark" ? "#020617" : "#ffffff"} />
    </section>
  );
}
