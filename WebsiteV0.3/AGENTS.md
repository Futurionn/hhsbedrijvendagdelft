...
5. **Colours, text sizes and motion speeds all come from
   ...
   (`text-display`/`title`/`title-sm`/`heading`/`lead`/`body`/`caption`/`kicker`)
   and the two durations (`duration-press`, `duration-settle`). The few places
...
6. **Two oranges.** `text-orange-deep dark:text-orange` for orange WORDS on
   ...
   for fills. The primary button uses a `text-navy-deep` label, never white.
...
9. **Use the shared UI primitives** in `src/shared/ui/` — `Button`, `Card`,
   `SectionHeading`, `HeroPill`, `Wave`. Do not hand-roll a styled `<button>`
   or `<a>`.
...
| Colours, type, motion | `tailwind.config.js` (+ `src/shared/theme.js`) |
